...
class CatalogView(ListAPIView):
    ...
    def get_queryset(self):
        ...
        data = filter_product(
            data_request
        )  # Преобразуем данные из request к словарю для фильтра
        products = Product.objects.filter(**data)  # Получаем продукты по фильтру
        products = filter_sorted(
            data_request, query=products
        )  # Сортируем продукты по сортировке
        return products
...
class TagsView(APIView):
    ...
    def get(self, request: HttpRequest) -> Response:
        ...
        if category_id is None:
            ...
            )  # Если нет id категории то возвращаем все тэги
...
class PopularView(APIView):
    ...
    def get(self, request: HttpRequest) -> Response:
        products = filter_popular_product(
            query=Product.objects.filter(available=True).order_by("sorted_index")[:8]
        )
        serializer = CatalogSerializer(products, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
