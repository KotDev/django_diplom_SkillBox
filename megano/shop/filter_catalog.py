...
def filter_product(data_request) -> dict:
    ...
    if data.get('category_id'):  # Проверяем удалилась ли категория из data
        if Category.objects.get(id=data.get('category_id')).parent_id:  # Проверяем  id категории на субкатегорию
            return data
        else:
            # Получаем все субкатегории категории
            data['category_id__in'] = list(Category.objects.filter(parent_id=data.get('category_id')))
...
def filter_sorted(data_request, query):
    ...
    if 'rating' in sort:  # Сортировка по рейтингу продукта
        ...
        ratings_list = sorted([(product.id, product.rating) for product in query], key=lambda x: x[1], reverse=reverse)
        # Сортируем нужные продукты по рейтингу
        pk_list = [idx for idx, rating in ratings_list]  # Получаем id отсортированных продуктов
        preserved = Case(*[When(pk=pk, then=pos) for pos, pk in enumerate(pk_list)])  # Создаём объект для order_by
        queryset = Product.objects.filter(pk__in=pk_list).order_by(preserved)  # Фильтруем продукты по id и order_by
        return queryset
