...
class BasketView(APIView):
    ...
    def get(self, request: HttpRequest) -> Response:
        ...
        product_list, count = create_product_list(
            cart
        )  # Достаём продукты из корзины и их кол-во
    ...
    def post(self, request: HttpRequest) -> Response:
        ...
        elif product.count <= cart.count_product(
            product_id
        ):  # Сверяем кол-во продукта в db и в корзине
            return Response(status=status.HTTP_400_BAD_REQUEST)
        else:
            cart.add(
                product=product, count=count_product
    ...
    def delete(self, request: HttpRequest) -> Response:
        ...
        if cart.count_product(product_id) == count_product:
            ...
            )  # Удаляем нужное ко-во продукта из корзины
