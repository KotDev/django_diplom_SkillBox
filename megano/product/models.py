...
class Product(models.Model):
    ...
    price = models.DecimalField(
        default=0, max_digits=10, db_index=True, decimal_places=2, verbose_name="Цена"
    )
    count = models.IntegerField(
        default=0, validators=[MinValueValidator(0)], verbose_name="Кол-во"
    )
    ...
    sorted_index = models.IntegerField(
        default=0,
    ...
    def rating(self):
        ...
        avg_rating = self.reviews.filter(product=self).aggregate(Avg("rate"))[
            "rate__avg"
        ]
...
class ProductImage(models.Model):
    ...
    src = models.ImageField(
        upload_to=product_image_directori_path,
        null=True,
        blank=True,
        verbose_name="Ссылка",
    )
